...
export default class Message extends Component {
  constructor (props) {
    super(props)
    this.featureCollection = JSON.parse(this.props.attributes.geo_information);
  ...
  _renderMapToggle() {
      if( this.featureCollection.features.length < 1) return null;
      return(
  ...
  _renderMap() {
    if( this.featureCollection.features.length < 1 || !this.state.showMap) return null;
    return(
        <Map center={[0, 0]} zoom={1} >
            <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
            <GeoJSON data={this.featureCollection} onAdd={this.onGeoInformationAdded} />
        </Map>
    );
}
